...
levels(col.iapv) <-  brewer.pal(nlevels(col.iapv), "Set3")
...
```{r}
glMDSPlot(lcpm, labels=paste(x$samples$group, x$samples$lane, x$samples$day, x$samples$mortality, x$samples$sbv, x$samples$iapv, x$samples$rnaConc, x$samples$rin, colnames(lcpm), sep="_"), groups=x$samples[,c(1,c(4:10))], launch=FALSE)
```
