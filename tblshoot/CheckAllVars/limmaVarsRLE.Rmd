...
x$samples$group <- unlist(lapply(colnames(x), function(x) substring(unlist(strsplit(x, "[.]"))[1],1)))
...
levels(col.mortality) <-  brewer.pal(nlevels(col.mortality), "Set1")
col.mortality <- as.character(col.mortality)

col.sbv <- x$samples$sbv
levels(col.sbv) <-  brewer.pal(nlevels(col.sbv), "Set2")
col.sbv <- as.character(col.sbv)

col.ipav <- x$samples$iapv
levels(col.ipav) <-  brewer.pal(nlevels(col.ipav), "Set3")
col.ipav <- as.character(col.ipav)

col.rnaConc <- x$samples$rnaConc
levels(col.rnaConc) <-  brewer.pal(nlevels(col.rnaConc), "Set1")
col.rnaConc <- as.character(col.rnaConc)

col.rin <- x$samples$rin
levels(col.rin) <-  brewer.pal(nlevels(col.rin), "Set2")
...
```{r}
glMDSPlot(lcpm, labels=paste(group, lane, day, mortality, sbv, iapv, rnaConc, rin, colnames(lcpm), sep="_"), groups=x$samples[,c(1,c(4:10))], launch=FALSE)
```
